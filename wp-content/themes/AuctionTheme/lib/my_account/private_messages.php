    ...
    <div class="my_box3">
		...
		<!-- page content here -->	
				...
				if(count($r) > 0)
				{
					...
					echo '</table>';
				} else _e('No messages here.','AuctionTheme');
      ...
      <?php if($owner == false): ?>
       <a href="<?php echo $privurl_m; ?>priv_act=delete-message&<?php
			...
			?>" class="nice_link"><?php _e("Confirm Deletion",'AuctionTheme'); ?></a> <?php endif; ?>
        ...
        <?php
			...
			if(isset($_POST['send']))
			{
				...
				$s = "selec t* from ".$wpdb->prefix."auction_pm where datemade='$tm' AND initiator='$myuid' AND user='$uid'";
				$r = $wpdb->get_results($s);
